...
export default function Vidlytics() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 pb-12 relative flex flex-col justify-between">
      <div>
        {/* 1. BARRA GLOBAL SLL HUB (Com ícone que volta para o painel principal + Seletor de Módulos) */}
        <div className="bg-white border-b border-slate-200 px-4 sm:px-6 py-2.5 z-40 sticky top-0 shadow-xs">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            {/* Ícone SLL (Voltar ao Hub Central) */}
            <div 
              onClick={() => navigate("/")}
              className="flex items-center gap-2.5 cursor-pointer group"
              title="Voltar ao Dashboard Central do SLL"
            >
              <img 
              ...
              <div className="hidden sm:flex flex-col">
                <span className="text-[11px] font-black tracking-tight text-slate-800 leading-none">SISTEMA LOJA LUCRATIVA</span>
                <span className="text-[10px] text-slate-400 font-medium leading-tight">Hub Central</span>
              </div>
            </div>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 hover:border-slate-300 bg-slate-50 hover:bg-slate-100 text-xs font-semibold text-slate-700 transition-colors"
              >
              ...
              {moduleMenuOpen && (
                <div className="absolute right-0 mt-1.5 w-56 bg-white rounded-xl shadow-lg border border-slate-200 py-1.5 z-50 animate-in fade-in slide-in-from-top-1">
                  <div className="px-3 py-1 text-[10px] uppercase font-bold text-slate-400">Alternar Módulo SLL</div>
                  {modules.map((m, idx) => (
                    <button
                      key={idx}
                      onClick={() => {
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 py-5 space-y-5">
          ...
          {/* 2. PROMO BANNER (Abaixo do Topo do Sistema, alinhado aos cards) */}
