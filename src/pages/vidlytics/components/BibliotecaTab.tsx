...
export const BibliotecaTab: React.FC = () => {
  ...
  // Dados mockados fiéis ao original
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      const newItem: MediaItem = {
        ...
        typeLabel: isVideo ? 'VÍDEO MP4 (HOSPEDADO)' : 'IMAGEM (HOSPEDADA)',
  ...
  const handleAddExternalVideo = (videoData: { title: string; url: string; platform: string }) => {
    const newItem: MediaItem = {
      id: String(Date.now()),
      name: videoData.title,
      type: 'video',
      typeLabel: `VÍDEO (${videoData.platform.toUpperCase()})`,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button 
            type="button" 
            className="flex items-center gap-1.5 px-3 py-2 bg-white border border-slate-200 hover:border-slate-300 rounded-xl text-xs font-bold text-slate-700 tracking-wider transition-colors shadow-sm cursor-pointer"
          >
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#0094eb] hover:bg-[#0082cf] text-white rounded-xl text-xs font-bold tracking-wider transition-colors shadow-md shadow-blue-500/20 cursor-pointer"
          >
            <UploadCloud size={16} />
                  ...
                  <td className="py-3 px-6 text-center">
                    <div className="inline-flex items-center justify-center gap-1.5 text-slate-400">
                      {media.type === 'video' && (
                        <button 
                          type="button"
                          className="p-1.5 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer"
                          title="Editar"
                        >
                          <Pencil size={15} />
                        </button>
                      ...
                      <button 
                        type="button"
                        className="p-1.5 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
                        title="Excluir"
                      >
                        <Trash2 size={15} />
                      </button>
      ...
      {/* Modal de URL Externa */}
      <AddVideoUrlModal
        isOpen={isUrlModalOpen}
        onClose={() => setIsUrlModalOpen(false)}
        onSuccess={handleAddExternalVideo}
      />
    </div>
  );
};
