...
const AparenciaModal: React.FC<AparenciaModalProps> = ({ isOpen, onClose, styleData }) => {
  ...
  const [styleName, setStyleName] = useState('Meu Estilo Padrão');
  ...
  // Lógica INJETADA: Carrega os dados e mapeia para os states corretos
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (style) {
          setFormData(style);
          if (style.name) setStyleName(style.name);
          if (typeof style.is_default !== 'undefined') setIsDefault(style.is_default);
          if (typeof style.is_unified !== 'undefined') setIsUnified(style.is_unified);
        } else {
          // Fallback seguro caso a loja ainda não tenha estilo salvo
          setFormData({ desktop: {}, mobile: {} });
        }
      } catch (error) {
  ...
  // Lógica INJETADA: Se ativar 'Unificado', copia os dados do desktop pro mobile para garantir simetria
  useEffect(() => {
    if (isUnified) {
      setFormData((prev: any) => ({
        ...prev,
        mobile: { ...(prev.mobile || {}), ...(prev.desktop || {}) }
      }));
    }
  }, [isUnified]);

  // Lógica INJETADA: Buscador seguro de configs
  const getConfig = (device: 'desktop' | 'mobile', key: string) => {
    return formData[device]?.[key] !== undefined ? formData[device][key] : '';
  };

  // Lógica INJETADA: Gravador de configs (salva nos dois se for unificado)
  const setConfig = (device: 'desktop' | 'mobile', key: string, value: any) => {
    setFormData((prev: any) => {
      const newState = { ...prev };
      
      if (isUnified) {
        newState.desktop = { ...(newState.desktop || {}), [key]: value };
        newState.mobile = { ...(newState.mobile || {}), [key]: value };
      } else {
  ...
  // Lógica INJETADA: Salva o estilo completo mesclando formulário + aba básico
