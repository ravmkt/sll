...
// Fallback UI para evitar erros de componentes não definidos
const selectClass = "mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-[#0094eb] focus:border-[#0094eb] sm:text-sm rounded-md bg-white";
const inputClass = "mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#0094eb] focus:border-[#0094eb] sm:text-sm bg-white";
...
const ColorInput = ({ value, onChange }: any) => (
  <div className="flex items-center gap-2">
    <input 
      ...
      className="h-10 w-10 cursor-pointer rounded border border-gray-300 p-1 bg-white"
    />
    <input 
      type="text" 
      value={value || ''} 
      onChange={(e) => onChange(e.target.value)}
      className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#0094eb] focus:border-[#0094eb] sm:text-sm bg-white uppercase"
      placeholder="#000000"
    />
  </div>
);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-900 w-[95vw] max-w-[1500px] h-[95vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-1 rounded-xl shrink-0 border border-slate-200 dark:border-slate-700">
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${
                  previewDevice === 'desktop' ? 'bg-white dark:bg-slate-700 text-[#0094eb] shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${
                  previewDevice === 'mobile' ? 'bg-white dark:bg-slate-700 text-[#0094eb] shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
        ...
        <div className="flex-1 flex overflow-hidden bg-slate-50 dark:bg-slate-900/50">
          ...
          {/* ESQUERDA - CONTROLES */}
          <div className="w-[450px] border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-y-auto p-6 flex flex-col gap-6 shrink-0 custom-scrollbar">
            ...
            {activeTab === 'basico' ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                    <label className="flex items-start gap-3 cursor-pointer">
                      <div className="mt-1">
                        <input type="checkbox" checked={isDefault} onChange={(e) => setIsDefault(e.target.checked)} className="w-5 h-5 rounded border-slate-300 text-[#0094eb] focus:ring-[#0094eb]" />
                      </div>
                      <div>
                  ...
                  <div className={`p-4 border rounded-xl transition-colors ${isUnified ? 'border-[#0094eb]/40 bg-[#0094eb]/5' : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50'}`}>
                    <label className="flex items-start gap-3 cursor-pointer">
                      <div className="mt-1">
                        <input type="checkbox" checked={isUnified} onChange={(e) => setIsUnified(e.target.checked)} className="w-5 h-5 rounded border-slate-300 text-[#0094eb] focus:ring-[#0094eb]" />
                      </div>
                      <div>
              ...
              <>
                ...
                <div className="flex items-center justify-between p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Editando Dispositivo:</span>
                    ...
                    <button 
                      onClick={() => {
                        if (!isUnified) {
                          ...
                          if(activeTab === 'carrossel') setCarouselDevice('desktop');
                          if(activeTab === 'flutuante') setFloatingDevice('desktop');
                        }
                      }}
                      className={`p-1 rounded-md transition-colors ${(isUnified || previewDevice === 'desktop') ? 'text-[#0094eb]' : 'text-slate-400'} ${isUnified ? 'cursor-default' : 'cursor-pointer'}`}
                    >
                    ...
                    <button 
                      onClick={() => {
                        if (!isUnified) {
                          ...
                          if(activeTab === 'carrossel') setCarouselDevice('mobile');
                          if(activeTab === 'flutuante') setFloatingDevice('mobile');
                        }
                      }}
                      className={`p-1 rounded-md transition-colors ${(isUnified || previewDevice === 'mobile') ? 'text-[#0094eb]' : 'text-slate-400'} ${isUnified ? 'cursor-default' : 'cursor-pointer'}`}
                    >
                ...
                {/* FORMULÁRIO DO CARROSSEL (Movido para a Esquerda) */}
                {activeTab === 'carrossel' && (
                    ...
                    <SectionCard title="Aparência do Carrossel" description="Estilo visual dos vídeos exibidos em formato de carrossel.">
                      <div className="grid grid-cols-2 gap-4">
                        <FormField label="Estilo dos Itens">
                          <select value={getConfig(carouselDevice, 'carousel_style') || 'stories'} onChange={e => setConfig(carouselDevice, 'carousel_style', e.target.value)} className={selectClass}>
                        ...
                        <FormField label="Tamanho dos Itens (px)">
                          <input type="number" min="40" max="200" value={getConfig(carouselDevice, 'carousel_item_size') || 80} onChange={e => setConfig(carouselDevice, 'carousel_item_size', parseInt(e.target.value) || 80)} className={inputClass} />
                        </FormField>
                        <FormField label="Espaçamento (Gap)">
                          <input type="number" min="0" max="50" value={getConfig(carouselDevice, 'carousel_gap') || 12} onChange={e => setConfig(carouselDevice, 'carousel_gap', parseInt(e.target.value) || 12)} className={inputClass} />
                        </FormField>
                        <FormField label="Cor da Borda">
                            <ColorInput value={getConfig(carouselDevice, 'carousel_border_color') || '#0094EB'} onChange={(v: string) => setConfig(carouselDevice, 'carousel_border_color', v)} />
                        </FormField>
                      </div>
                    </SectionCard>
                    ...
                    <SectionCard title="Posicionamento na Página" description="Onde o carrossel será injetado no site da loja.">
                      <div className="grid grid-cols-1 gap-4">
                        <FormField label="Posição Padrão">
                          <select value={getConfig(carouselDevice, 'carousel_position') || 'top'} onChange={e => setConfig(carouselDevice, 'carousel_position', e.target.value)} className={selectClass}>
                        ...
                        <FormField label="Seletor HTML (Se customizado)">
                          <input type="text" placeholder="ex: #meu-carrossel" value={getConfig(carouselDevice, 'carousel_custom_selector') || ''} onChange={e => setConfig(carouselDevice, 'carousel_custom_selector', e.target.value)} className={inputClass} />
                        </FormField>
                      </div>
                    </SectionCard>
                  </div>
                ...
                {/* FORMULÁRIO DO FLUTUANTE (Movido para a Esquerda) */}
                {activeTab === 'flutuante' && (
                  <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <SectionCard title="Aparência do Widget" description="Como a bolinha flutuante será exibida no site.">
                      <div className="grid grid-cols-2 gap-4">
                        <FormField label="Posição na Tela">
                          <select value={getConfig(floatingDevice, 'floating_position') || 'bottom-right'} onChange={e => setConfig(floatingDevice, 'floating_position', e.target.value)} className={selectClass}>
                            <option value="bottom-left">Inferior Esquerda</option>
                            <option value="bottom-right">Inferior Direita</option>
                          </select>
                        </FormField>
                        <FormField label="Tamanho da Bolinha (px)">
                          <input type="number" min="40" max="120" value={getConfig(floatingDevice, 'floating_size') || 80} onChange={e => setConfig(floatingDevice, 'floating_size', parseInt(e.target.value) || 80)} className={inputClass} />
                        </FormField>
                        <FormField label="Arredondamento (px)">
                          <input type="number" min="0" max="100" value={getConfig(floatingDevice, 'floating_border_radius') || 100} onChange={e => setConfig(floatingDevice, 'floating_border_radius', parseInt(e.target.value) || 0)} className={inputClass} />
                        </FormField>
                        <FormField label="Espessura da Borda">
                          <input type="number" min="0" max="10" value={getConfig(floatingDevice, 'floating_border_width') || 2} onChange={e => setConfig(floatingDevice, 'floating_border_width', parseInt(e.target.value) || 0)} className={inputClass} />
                        </FormField>
                        <FormField label="Sombra Externa">
                          <select value={getConfig(floatingDevice, 'floating_shadow') || 'md'} onChange={e => setConfig(floatingDevice, 'floating_shadow', e.target.value)} className={selectClass}>
                        ...
                        <FormField label="Cor da Borda">
                            <ColorInput value={getConfig(floatingDevice, 'floating_border_color') || '#0094EB'} onChange={(v: string) => setConfig(floatingDevice, 'floating_border_color', v)} />
                        </FormField>
                      </div>
                    </SectionCard>
                    ...
                    <SectionCard title="Espaçamento" description="Distância da bolinha em relação aos cantos da tela.">
                      <div className="grid grid-cols-2 gap-4">
                        <FormField label="Margem Inferior (px)">
                          <input type="number" min="0" max="200" value={getConfig(floatingDevice, 'floating_margin_bottom') || 20} onChange={e => setConfig(floatingDevice, 'floating_margin_bottom', parseInt(e.target.value) || 0)} className={inputClass} />
                        </FormField>
                        <FormField label="Margem Lateral (px)">
                          <input type="number" min="0" max="200" value={getConfig(floatingDevice, 'floating_margin_side') || 20} onChange={e => setConfig(floatingDevice, 'floating_margin_side', parseInt(e.target.value) || 0)} className={inputClass} />
                        </FormField>
                      </div>
                    </SectionCard>
                  </div>
                ...
                {/* FALLBACK PARA ABAS EM DESENVOLVIMENTO (Evita tela branca) */}
                {['carrossel-dinamico', 'grade', 'player'].includes(activeTab) && (
                   <div className="p-6 border border-dashed border-slate-300 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30 flex flex-col items-center justify-center text-center">
                      <Settings2 size={32} className="text-slate-400 mb-3" />
                      <p className="font-bold text-slate-700 dark:text-slate-300">Controles em Desenvolvimento</p>
                      <p className="text-sm text-slate-500 mt-1">Os controles para {activeTab.replace('-', ' ')} serão adicionados aqui em breve.</p>
                   </div>
                )}
              </>
            )}
          ...
          {/* DIREITA - ÁREA DE PREVIEW (Agora limpa, apenas mostrando o mockup) */}
          <div className="flex-1 flex items-center justify-center p-8 relative overflow-hidden h-full">
            
            {activeTab === 'basico' ? (
              // PREVIEW BÁSICO IDÊNTICO AO SEU DESIGN
              ...
              // ÁREA DE PREVIEW (MOCKUPS LIMPOS)
                ...
                {previewDevice === 'desktop' ? (
                  <div className="w-full max-w-5xl aspect-video bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden">
                     ...
                     <div className="flex-1 relative bg-slate-50 dark:bg-slate-900 flex items-center justify-center overflow-hidden">
                        <span className="z-10 text-slate-400 font-bold uppercase tracking-widest text-center px-4">
                           Preview do {activeTab.replace('-', ' ')} (Desktop)
                        </span>
                     </div>
                  </div>
                ) : (
                  <div className="h-full max-h-[800px] aspect-[9/19] rounded-[2.5rem] border-[10px] border-[#1a1f36] bg-slate-50 dark:bg-slate-900 shadow-2xl relative flex items-center justify-center overflow-hidden shrink-0">
                    ...
                    <span className="z-10 text-slate-400 text-sm font-bold uppercase tracking-widest text-center px-4">
                       Preview do {activeTab.replace('-', ' ')} (Mobile)
                    </span>
                  </div>
                )}
              </div>
