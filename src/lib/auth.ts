...
import { db, resolveStoreId, isSupabaseConfigured } from './db';
...
export const ensureUserTenantAtomics = async (user: any, customStoreName?: string) => {
  ...
  const userName =
    user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split('@')[0] || 'Empresa';
  ...
  try {
    ...
    if (!error && data?.store_id) {
      ...
      if (typeof window !== 'undefined' && window.localStorage) {
        localStorage.setItem('sll_current_store_id', storeId);
    ...
    if (error) {
      console.warn('[Auth] Falha na RPC create_or_get_user_tenant:', error.message);
    }
  } catch (rpcErr) {
  ...
  if (existingStore?.id) {
    if (typeof window !== 'undefined' && window.localStorage) {
      localStorage.setItem('sll_current_store_id', existingStore.id);
    }
...
export const createInitialTenantForUser = async ({
  ...
  if (referralCode) {
    try {
      const { data: refStore } = await supabase
        ...
        .maybeSingle();
      if (refStore?.id) referredByStoreId = refStore.id;
    } catch (e) {
  ...
  if (memberError) console.warn('[Auth] Aviso ao vincular membro:', memberError);
  ...
  if (settingsError) console.warn('[Auth] Aviso ao criar store_settings padrão:', settingsError);
...
export const resolveCurrentStoreId = async () => {
  ...
  if (store?.id) {
    if (typeof window !== 'undefined' && window.localStorage) {
      localStorage.setItem('sll_current_store_id', store.id);
    }
    return store.id;
  }
